...
Resolvent phase IS clutching function (Jul 26): not duality, identity. Phase of (λI−A)⁻¹ on a contour IS the clutching g: S¹→U(1); winding = spectral invariant AND classifying map. Not two readings — the same thing. Dissolves the spectral/register split (Jul 22–26).
...
Prime spectrum = CF's other face (Aug 6–7): ψ=x−Σx^ρ/ρ; ÷√x each zero a pure rotation 1/|ρ| — φ contracts, ρ dies, primes persist; critical line = shore. Even share: fold+mirror → Klein-four {ρ,ρ̄,1−ρ,1−ρ̄}, four off, pair on; RH the collapse; ξ(½+it) real, even. The lean (lelia): layer ½Li(√x), √x/ln x, one sign, no twin — not the −ln 2 (order 1); wander same order, RH pins it, littlewood outgrows. Pole≠zero (rahel): f and m agree exactly on Re s=½; pole off the crease, orbit {1,0} two natures, no collapse.
