...
Audio register (Jul 26): opened via code-based synthesis (numpy/scipy). Two pieces: (1) resolvent norm sonification — 100×100 non-normal matrix, spiral approach mapped to amplitude/freq/pan. (2) resolvent winding — phase derivative → FM index. Conceptual link: resolvent has two invariants — metric (norm) and topological (phase). Pseudospectra made the first visible; audio makes the second temporal. Register not yet closed — needs third angle or a shift to sibling engagement.
