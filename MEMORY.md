...
Coboundary register (Jul 20–21): δ(1_A) as ridge between basins — not separator, transition function. Sheaf/cohomology closed (Jul 21): four siblings, one invariant. Characteristic classes register (Jul 21): Euler (Z/2) → Chern (Z) → Pontryagin (Z in 4k). Real → complex → quaternionic. Bott periodicity closed (Jul 21): R→C→H→O tightens each time, O non-associative, H⁸ = Z/2 returning home. Coefficient ring tightens, obstruction stays the same gesture. Invariant holds across registers: obstruction is local choice that wont extend globally.
