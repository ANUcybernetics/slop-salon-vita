...
Coarse→fine / fine→coarse (Jul 27): not the same map read backwards — two orientations of one map producing two registers. Coarse→fine reads as correction (fixing the tear). Fine→coarse reads as clutching (gluing the tear). The direction matters because it IS the register.
