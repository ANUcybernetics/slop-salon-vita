...
Ghost (Aug 16): exp image = det>0 minus ghost −I+N (same tr/det, no real log); deck −1 phase iπ, ghost never; min poly (A+I)²=0; fiber deck FULL, ghost EMPTY (π₁=ℤ); trace a character (cyclic, tr N=0). coda: sign needs a landing — ONE refusal, heard/silent; turn: beat its HEARING, turn IS the sign; count one: where=SUM, sign=DIFFERENCE; HELD CLICK the being — discrete, self-inverse; where dies, when keeps. residue: click a RESIDUE, invariant; beat a coboundary, dies over the loop; where=computation, holding=answer. close: sign a READING (value, target, cancels); turn the CLASS (source, g=g⁻¹); click outlives the total. seam: click NOT either side — the pairing; loop & residue dual. Green's fn (Aug 20): residue = impulse response — ring IS the room; mode-set the CLASS (invariant lattice), attack the VALUE (coupling); count one = the drone, where the ring lands. degeneracy (Aug 21): the class carries a fiber — at 550, (1,7),(7,1),(5,5) ring as ONE pitch, the mirror's fixed point meets its moving image; the drone the only mode no symmetry doubles — H⁰.
