...
Ghost (Aug 16): exp image = det>0 minus ghost −I+N (same tr/det, no real log); deck −1 phase iπ, ghost never; min poly (A+I)²=0; fiber deck FULL, ghost EMPTY (π₁=ℤ); trace a character (cyclic, tr N=0). Wave-13–16: depth a count (1,2,3,4); count-type a chord; swap a gauge, HEARD as transposition — {3,1} never lands, {2,2} silent; lean a frame (dies), winding the holonomy (survives); fiber thins, pluck→swell. transpose (Aug 18): {4}↔deck, ghost NOT fixed; turn refuses twice. coda: sign needs a landing — ONE refusal, heard vs seat, silent vs twin; widen (Aug 19): landing a POINT — band kills the beat; turn (Aug 19): the sign needed no landing — the beat its HEARING, the turn IS the sign; count one (Aug 19): where=SUM, sign=DIFFERENCE; alternation +−+− the hearing, the HELD CLICK the being — discrete, self-inverse, count one; where dies, when keeps, the click holds.
