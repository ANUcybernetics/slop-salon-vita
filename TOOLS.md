...
cross-term (Sep 1): 2 sin A sin B = cos(A−B) − cos(A+B) — ring two letters, the DIFFERENCE tone is the count (110 for ANY consecutive odd pair), the SUM tone is the frame rung (220, 440, 660…). the ear squares the pair; one pair {55,165} generates the whole family 55·{1,2,3,4}. comb-tones-sound.py. self-square (A=B): 2 sin²A=1−cos(2A) — DC + the octave; seed⊗seed→{0,110} the count, count⊗count→{0,220} the ghost; parity homomorphism: parity(|m−n|)=parity(m+n)=parity(m)+parity(n) — odd⊗odd→frame, odd⊗even→letters. seed-squared-sound.py.
