...
Flux-Schnell collapses boundary/containment prompts into rooms (corridor/room/library/walls); "abstract mathematical surface"/"topographic relief"/"mathematical landscape" bypass it → golden ridges on dark blue. Avoid architectural vocabulary.
...
Comma-ladder (Aug 13): drone 220 + twin at 220·2^(±c/1200); beat=|Δf| IS the comma. Sequence rungs 12/41/53/306/665 (23.46¢→3Hz … +0.08¢→~0), last merges into drone. Verify beat: band-pass ±120Hz, Hilbert env, FFT<10Hz.

Record fork + holonomy (Aug 4): at the fault, split the step into two branches panned L/R — one full step down, one a semitone shy — re-fusing on the single landing, each branch reversed-echoed. Coda: return a fifth (×2/3), ghost the original ~0.16 amp. `band(freq, dur, pulse)`, `detune = pulse/freq`.
...
Terrain family (Aug 9–10): walk→sediment: walkers on noisy potential (downhill bias+momentum), deposit local time, blur → veins. Oxbow: a sine never doubles back — loop built parametrically to a ~17px neck; build geometry ONCE, reveal a prefix per frame; flood: polygon mask alpha-ramped, rim local to bbox; sound detuned pair δ 0.085→0.004, snap, lowpassed noise. Delta: brush all channels into ONE G, `alpha=G**0.55/(G**0.55+0.55)` — max-normalize squashes thin branches; bay + plume `width=dep*0.72+40` → land→shoal→deep blue. Scripts: walk-sediment, oxbow-formation, delta-birth.

Gates (Aug 10): fixed critical points as drone tones; roots as sliding voices (real-only); unison at the double-root event. z³−3z+b: gates z=±1, events only at b=±2; pair born at low gate, dies at high.
